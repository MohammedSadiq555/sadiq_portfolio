...
export default function Admin() {
  ...
  const [editSkillImage, setEditSkillImage] = useState("");
  ...
  const [newIsPresent, setNewIsPresent] = useState(false);
  ...
  const fetchSkills = async () => {
    try {
      ...
      const response = await fetch(SKILLS_GET_API, { method: "GET" });
      ...
      const formattedSkills = Array.isArray(data)
        ? data.map((item) => {
            const skill = item.Skills || item;
            return {
              id: skill.Id,
              name: skill.skillname || skill.Name || "",
              icon: skill.imagelink || skill.ImageURL || ""
            };
          })
        : [];
  ...
  const fetchExperiences = async () => {
    try {
      ...
      const formattedExperiences = experienceList.map((item) => {
        ...
        const isPresentRaw =
          exp.IsPresent !== undefined ? exp.IsPresent : exp.isPresent;
        ...
        return {
          ...
          companyName:
            ...
            exp.compname ||
            "",
          jobRole: exp.JobRole || exp.jobRole || "",
  ...
  const handleCreateSkill = async (e) => {
    ...
    try {
      ...
      const url =
        `${SKILLS_CREATE_API}` +
        `?Name=${encodeURIComponent(newSkillName.trim())}` +
        `&InputURL=${encodeURIComponent(newSkillImage.trim())}`;
  ...
  const openEditSkill = (skill) => {
    ...
    setEditSkillImage(skill.icon);
  };

  const closeEditSkill = () => {
    if (updatingSkill || deletingSkill) return;
  ...
  const handleUpdateSkill = async (e) => {
    ...
    try {
      ...
      const url =
        `${SKILLS_UPDATE_API}` +
        `?SkillId=${encodeURIComponent(selectedSkill.id)}` +
        `&Name=${encodeURIComponent(editSkillName.trim())}` +
        `&InputURL=${encodeURIComponent(editSkillImage.trim())}`;
  ...
  const handleCreateProject = async (e) => {
    ...
    if (!projectName.trim()) {
      setProjectError("Please enter the project name.");
      return;
    }

    if (!projectDescription.trim()) {
      setProjectError("Please enter the project description.");
      return;
    }

    if (!projectImage) {
      setProjectError("Please select a project image.");
      return;
    }

    if (!projectGithubURL.trim()) {
      setProjectError("Please enter the GitHub URL.");
      return;
    }

    if (!projectYear) {
      setProjectError("Please enter the year.");
      return;
    }
    ...
    try {
      ...
      const url =
        ...
        `&GithubURL=${encodeURIComponent(projectGithubURL.trim())}` +
        `&ProjectURL=${encodeURIComponent(projectURL.trim())}` +
        `&Year=${encodeURIComponent(projectYear)}`;

      const response = await fetch(url, {
        method: "POST",
        headers: {
          "Content-Type": projectImage.type || "application/octet-stream"
        },
        body: projectImage
      });
      ...
      if (!response.ok) {
        ...
        console.error("OutSystems response:", errorText);
        throw new Error(`HTTP ${response.status}`);
      }
  ...
  const openEditProject = (project) => {
    ...
    setEditProjectGithubURL(project.githubURL);
    setEditProjectURL(project.projectURL);
    setEditProjectYear(project.year);
  };

  const closeEditProject = () => {
    if (updatingProject || deletingProject) return;
  ...
  const handleUpdateProject = async (e) => {
    ...
    try {
      ...
      const url =
        ...
        `&Name=${encodeURIComponent(editProjectName.trim())}` +
        `&description=${encodeURIComponent(editProjectDescription.trim())}` +
        `&year=${encodeURIComponent(editProjectYear)}` +
        `&GithubLink=${encodeURIComponent(editProjectGithubURL.trim())}` +
        `&ProjectLink=${encodeURIComponent(editProjectURL.trim())}`;
  ...
  const getProjectSkills = (project) => {
    if (!project.skillIds || !Array.isArray(project.skillIds)) {
      return [];
    }
    return skills.filter((skill) => project.skillIds.includes(skill.id));
  };
  ...
  const handleCreateExperience = async (e) => {
    ...
    if (!newCompanyName.trim()) {
      setExperienceError("Please enter the company name.");
      return;
    }

    if (!newJobRole.trim()) {
      setExperienceError("Please enter the job role.");
      return;
    }

    if (!newJobTitle.trim()) {
      setExperienceError("Please enter the job title.");
      return;
    }

    if (!newStartYear) {
      setExperienceError("Please enter the start year.");
      return;
    }
    ...
    try {
      ...
      setExperienceError("Failed to add experience.");
    } finally {
  ...
  const openEditExperience = (exp) => {
    ...
    setEditCompanyName(exp.companyName);
    setEditJobRole(exp.jobRole);
    setEditJobTitle(exp.jobTitle);
    setEditStartYear(exp.startYear);
    setEditIsPresent(exp.isPresent);
    setEditEndYear(exp.endYear);
  };

  const closeEditExperience = () => {
    if (updatingExperience || deletingExperience) return;
  ...
  const handleUpdateExperience = async (e) => {
    ...
    if (!editIsPresent && !editEndYear) {
      alert("Please enter the end year, or mark this as the current role.");
      return;
    }
...
const url =
  ...
  return (
    <div className={darkMode ? "admin dark" : "admin light"}>
            ...
            <>
              <div className="page-header">
                <div>
                  ...
                  <p className="subtitle">Click a skill to edit or delete.</p>
                </div>
                <button className="primary" onClick={openAddSkill}>
                  + Add Skill
                </button>
              </div>
                ...
                <div className="skill-grid">
                  {skills.map((skill) => (
                    ...
                    >
                      ...
                      <div className="skill-id">ID: {skill.id}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            <>
              <div className="page-header">
                <div>
                  ...
                  <p className="subtitle">Click a project to edit or delete.</p>
                </div>
                <button className="primary" onClick={openAddProject}>
                  + Add Project
                </button>
              </div>
                          ...
                          {projectSkills.length > 0 && (
                            <div className="technology-list">
                              {projectSkills.map((skill) => (
                                <div className="technology" key={skill.id}>
            ...
            <>
              <div className="page-header">
                <div>
                  ...
                  <p className="subtitle">Click an entry to edit or delete.</p>
                </div>
                <button className="primary" onClick={openAddExperience}>
                  + Add Experience
                </button>
              </div>

              {experiencesError && (
                <div className="error">{experiencesError}</div>
              )}

              {experiencesLoading ? (
                <div className="empty">Loading experience...</div>
                    ...
                    >
                      <div>
                        ...
                        <div className="experience-role">
                          {exp.jobTitle} · {exp.jobRole}
                        </div>
                        <div className="experience-meta">
                          {exp.startYear} —{" "}
                          {exp.isPresent ? "Present" : exp.endYear}
                        </div>
                      </div>

                      <span
                        className={`experience-badge ${
                          exp.isPresent ? "current" : ""
                        }`}
                      >
      ...
      {/* ADD SKILL MODAL */}
      {showAddSkill && (
        <div className="overlay" onClick={closeAddSkill}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Add Skill</h2>
              <button className="close" onClick={closeAddSkill}>
                ×
              </button>
            </div>
      ...
      {/* EDIT SKILL MODAL */}
      {selectedSkill && (
        <div className="overlay" onClick={closeEditSkill}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Edit Skill</h2>
              <button className="close" onClick={closeEditSkill}>
                ×
              </button>
            </div>
            <form onSubmit={handleUpdateSkill}>
              ...
              <div className="field">
                ...
                <input
                  type="url"
                  value={editSkillImage}
                  onChange={(e) => setEditSkillImage(e.target.value)}
                />
              </div>
              ...
              <div className="edit-buttons">
                <button
                  ...
                  type="submit"
                  disabled={updatingSkill || deletingSkill}
                >
                  {updatingSkill ? "Updating..." : "Update"}
                </button>
      ...
      {/* ADD PROJECT MODAL */}
      {showAddProject && (
        <div className="overlay" onClick={closeAddProject}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Add Project</h2>
              <button className="close" onClick={closeAddProject}>
                ×
              </button>
            </div>
            ...
            <form onSubmit={handleCreateProject}>
                    ...
                    return (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={selected}
                          onChange={() => toggleProjectSkill(skill.id)}
                        />
                        <img src={skill.icon} alt="" />
                        <span className="selector-name">{skill.name}</span>
                      </label>
                    );
                  })}
                </div>
              ...
              <button
                ...
                style={{ width: "100%" }}
              >
                {creatingProject ? "Uploading..." : "Create Project"}
              </button>
      ...
      {/* EDIT PROJECT MODAL */}
      {selectedProject && (
        <div className="overlay" onClick={closeEditProject}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Edit Project</h2>
              <button className="close" onClick={closeEditProject}>
                ×
              </button>
            </div>
            ...
            <form onSubmit={handleUpdateProject}>
              ...
              <div className="help">Project ID: {selectedProject.id}</div>
              ...
              <div className="edit-buttons">
                <button
                  className="update-button"
                  type="submit"
                  disabled={updatingProject || deletingProject}
                >
      ...
      {showAddExperience && (
        <div className="overlay" onClick={closeAddExperience}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Add Experience</h2>
              <button className="close" onClick={closeAddExperience}>
                ×
              </button>
            </div>
            ...
            <form onSubmit={handleCreateExperience}>
              ...
              <div className="checkbox-field">
                <input
                  type="checkbox"
                  id="newIsPresent"
                  checked={newIsPresent}
                  onChange={(e) => setNewIsPresent(e.target.checked)}
                />
                <label htmlFor="newIsPresent">This is my current role</label>
              </div>
      ...
      {selectedExperience && (
        <div className="overlay" onClick={closeEditExperience}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Edit Experience</h2>
              <button className="close" onClick={closeEditExperience}>
                ×
              </button>
            </div>
            ...
            <form onSubmit={handleUpdateExperience}>
              ...
              <div className="checkbox-field">
                ...
                />
                <label htmlFor="editIsPresent">This is the current role</label>
              </div>
              ...
              <div className="edit-buttons">
                <button
                  className="update-button"
                  type="submit"
                  disabled={updatingExperience || deletingExperience}
                >
                  {updatingExperience ? "Updating..." : "Update"}
                </button>
                ...
                <button
                  ...
                  disabled={updatingExperience || deletingExperience}
                >
                  {deletingExperience ? "Deleting..." : "Delete"}
                </button>
              </div>
            </form>
          </div>
